...
function handleInput() {
    ...
    if (!input) {
        showTip("❌Please enter a domain name or keyword！","error");
        return;
    }
    ...
    if (isFullURL) {
        try {
            ...
            if (fileExtensions.test(urlObj.pathname) && !isProbablyDomain) {
                showTip("📦 Recognized as a file link, jumping...", "info");
                delayedOpen(input);
                return;
            }
        } catch {}
    ...
    try {
        new URL(input);
        showTip("🌐 Recognized as a full link, jumping around...", "success");
        delayedOpen(input);
        return;
    } catch {}
    ...
    if (domainRegex.test(input) || ipRegex.test(input)) {
        ...
        showTip("🔗 Recognized as a domain/IP address, jumping...", "success");
        delayedOpen(finalURL);
        return;
    }

    // 搜索
    showTip("🔍 Unrecognized format, search engine queries...", "info");
...
function checkZoom() {
    ...
    if (zoom !== 100) {
        showTip("⚠️ For the best experience, please keep the page zoom at 100%.", "info", 3000);
    }
}
