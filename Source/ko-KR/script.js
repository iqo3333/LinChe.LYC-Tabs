...
function handleInput() {
    ...
    if (!input) {
        showTip("❌ 도메인 이름 또는 키워드를 입력하세요.！","error");
        return;
    }
    ...
    try {
        new URL(input);
        showTip("🌐 전체 링크로 인식되어 점프하고 있습니다....", "success");
        delayedOpen(input);
        return;
    } catch {}
    ...
    if (domainRegex.test(input) || ipRegex.test(input)) {
        ...
        showTip("🔗 도메인/IP 주소로 인식, 점프...", "success");
        delayedOpen(finalURL);
        return;
    }

    // 搜索
    showTip("🔍 인식할 수 없는 형식, 검색 엔진 쿼리...", "info");
...
function checkZoom() {
    ...
    if (zoom !== 100) {
        showTip("⚠️ 최고의 경험을 위해 페이지 확대/축소 비율을 100%로 유지해 주세요.", "info", 3000);
    }
}
